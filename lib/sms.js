...
var KueWorker = require(path.join(__dirname, 'kue', 'worker'));
...
SMSSchema.statics.queue = function (sms, options, done) {
  ...
  SMS._send(sms, function (error, _sms) {
    ...
    if (error) {
      ...
      //notify `SMS:queued` kue worker if available
      //to process queued SMS notification
      if (SMS._queue) {

        //deduce kue queue name
        var queueName = (_config.kue || {}).queue || 'SMS:queued';

        //create SMS notification job
        //and queue it
        var job = SMS._queue.create(
          queueName,
          _sms.toObject()
        );
...
try {
  ...
  if (_config.kue && _.isPlainObject(_config.kue)) {
    ...
    SMSSchema.statics._queue = kue.createQueue(_config.kue);

    //exports kue job processing worker
    SMSSchema.statics.worker = KueWorker;
